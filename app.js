...
function renderResult() {
  ...
  if (candidate.extraction) {
    ...
    const facts = (answerCandidate?.known_facts || []).map(item => `<li><b>${escapeHtml(item.label)}</b>：${escapeHtml(item.value)}<small>${escapeHtml(item.basis)}</small></li>`).join('') || '<li>截图中没有可确认的商品字段。</li>';
    ...
    const uncertainties = (answerCandidate?.decision_uncertainties || []).map(item => `<li><b>${escapeHtml(item.label)}</b>：${escapeHtml(item.why_it_matters)}</li>`).join('') || '';
    const decision = mvpDecision(candidate);
